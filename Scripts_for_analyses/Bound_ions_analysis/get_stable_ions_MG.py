...
def process_system_and_run(system_name, topology_file, trajectory_file, run_index):
        ...
        if region != 'Interface':
            ...
            if region == 'Backbone':
                if all(ion_index not in df.loc[df['Region'] == r, 'Ion_Resid'].values 
                    ...
                    classifications['DNA_backbone'].append((ion_index, binding_info))

            elif region == 'Gap_Region': 
                if all(ion_index not in df.loc[df['Region'] == r, 'Ion_Resid'].values 
                       for r in ['Major_Groove', 'Minor_Groove']):
                    classifications['DNA_gap'].append((ion_index, binding_info))

            elif region == 'Major_Groove':
                if all(ion_index not in df.loc[df['Region'] == r, 'Ion_Resid'].values 
                       for r in ['Minor_Groove', 'Histone_Tail', 'Histone_Core', 'Interface']):
                    classifications['major_groove'].append((ion_index, binding_info))

            elif region == 'Minor_Groove':
                if all(ion_index not in df.loc[df['Region'] == r, 'Ion_Resid'].values 
                       for r in ['Major_Groove', 'Histone_Tail', 'Histone_Core', 'Interface']):
                    classifications['minor_groove'].append((ion_index, binding_info))

            elif region == 'Histone_Tail':
                if all(ion_index not in df.loc[df['Region'] == r, 'Ion_Resid'].values 
                       for r in ['Backbone', 'Major_Groove', 'Minor_Groove', 'Interface']):
                    classifications['histone_tail'].append((ion_index, binding_info))

            elif region == 'Histone_Core':
                if all(ion_index not in df.loc[df['Region'] == r, 'Ion_Resid'].values 
                       for r in ['Backbone', 'Major_Groove', 'Minor_Groove', 'Interface']):
                    classifications['histone_core'].append((ion_index, binding_info))            
            ...
            classifications['DNA-histone_Interface'].append((ion_index, dna_list, histone_list))
    ...
    for classification, items in classifications.items():
        if classification == 'DNA-histone_Interface':
            ion_bindings = {}
...
systems = [
    {
        "name": "MG_15mM_1264_Box_50",
        "topology": "/media/lenovo/Seagate Basic/remove_wat_nc_40ps/MG_15mM_1264_Box_50_rw.prmtop",
        "trajectory_files": [
            "/media/lenovo/Seagate Basic/remove_wat_nc_40ps/MG_15mM_1264_Box_50_run1_rw.nc",
            "/media/lenovo/Seagate Basic/remove_wat_nc_40ps/MG_15mM_1264_Box_50_run2_rw.nc",
            "/media/lenovo/Seagate Basic/remove_wat_nc_40ps/MG_15mM_1264_Box_50_run3_rw.nc"
        ]
    },
]
