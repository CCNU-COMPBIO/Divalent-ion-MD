...
#align.AlignTraj(u, u, select='(nucleic and name P) ', in_memory=True).run() 
ref_u = MDAnalysis.Universe(struc)  # 只加载结构文件，不加载轨迹文件
# 使用结构文件作为参考进行对齐

align.AlignTraj(u, ref_u, select='(protein and name CA) or (nucleic and name P)', reference_frame=0, in_memory=True).run()
...
cap = u.select_atoms('(protein and name CA)') + u.select_atoms('nucleic and not (resid 1:10 or resid 138:147 or resid 285:294 or resid 148:157) and name P')
n_atoms = len(cap)

cap_ref = ref_u.select_atoms('(protein and name CA)') + ref_u.select_atoms('nucleic and name P')
...
#R = pa.T
#cap1 = u.select_atoms('(protein and name CA)') + u.select_atoms('nucleic and name P')
...
for i in range(divisor):
   ...
   #rmsf_data = RMSF(cap, start=start_n + start, stop=stop_n + start).run()
...
# 自动命名保存文件
output_csv = f"./csv/{prefix}_height_radius.csv"
